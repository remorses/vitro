 ...
 */
export function mdxTransformer(file, api) {
    ...
    function genObjectExpression(attrs) {
        return j.objectExpression(
            Object.entries(attrs).map(([key, val]) =>
                j.property('init', j.identifier(key), val),
            ),
        )
    }
    ...
    function convertToStories(path) {
        ...
        base.find(j.JSXElement)
            ...
            .forEach((elt) => {
                const attrs: any = parseJsxAttributes(
                    elt.node.openingElement.attributes,
                )
                if (attrs.name) {
        ...
        if (root.find(j.ExportNamedDeclaration).size() > 0) {
            meta.includeStories = j.arrayExpression(
                includeStories.map((key) => j.literal(key)),
            )
        }
        const statements = [
    ...
    root.find(j.ImportDeclaration)
        .at(0)
        .replaceWith((exp) =>
            j.importDeclaration(exp.node.specifiers, exp.node.source),
        )

    //  import { Story, Meta } from '@storybook/addon-docs/blocks';
    root.find(j.ImportDeclaration)
        .filter(
            (exp) => exp.node.source.value === '@storybook/addon-docs/blocks',
        )
        .remove()
    ...
    root.find(j.VariableDeclaration)
        .filter(
            (decl) =>
                ...
                MDX_DECLS.includes(decl.node.declarations[0].id.name),
        )
        .remove()
